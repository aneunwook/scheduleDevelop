...
public class CommentController {
    ...
    @PostMapping
    public ResponseEntity<CommentCreateResponseDto> createComment(@RequestBody @Valid CommentCreateRequestDto requestDto, HttpSession session){

        User user = (User) session.getAttribute("user");
        CommentCreateResponseDto comment = commentService.createComment(user.getId(), requestDto);
